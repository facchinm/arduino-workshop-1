...
void keyReleased() {
  key_Strokes++;
  int rcnt_KeyStroke = keyboard.getOemKey();
  SERIAL_PORT_MONITOR.print(rcnt_KeyStroke);

  if (rcnt_KeyStroke != 40) {
    SERIAL_PORT_MONITOR.print("Released: ");
    payload[key_Strokes] = rcnt_KeyStroke;
    printKey();
  }
  else {
    SERIAL_PORT_MONITOR.print(" Message to be sent!");
    SigFox.begin();
    delay(100);
    SigFox.beginPacket();
    /*
